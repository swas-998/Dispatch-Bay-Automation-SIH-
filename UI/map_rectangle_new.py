...
class Maps:
    ...
    def back_map(self):   
        ...
        self.ball=self.back.create_oval(40-5,140-5,50+5,150+5,fill='red',width=2)

    def moov(self,cord):
        r=int(cord.getvalx())
        c=int(cord.getvaly())
        cord.count=cord.count+1
...
if __name__ == '__main__':
    ...
    ob.m(c)
    cord=Cordinate()
    t2=threading.Thread(target=filling,args=(cord,))
    t2.start()
    print('hello')
    while True:
        if cord.count>3:
